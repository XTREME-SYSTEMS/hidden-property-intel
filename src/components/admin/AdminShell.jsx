...
import {
  ...
  ChevronDown, Compass, Briefcase, FileSignature, Wrench, Network, Brain, Menu
} from "lucide-react";
...
export default function AdminShell() {
  ...
  return (
    <div className="relative flex h-[calc(100vh-112px)] overflow-hidden border border-black/10">
      ...
      <aside
        className={`absolute inset-y-0 left-0 z-50 flex w-72 shrink-0 flex-col bg-[#0c0d0e] text-white transition-transform duration-200 md:relative md:translate-x-0 md:w-64 ${
      ...
      >
        <div className="flex items-center gap-2.5 border-b border-white/10 px-5 py-4">
          ...
          <button
            onClick={() => setSidebarOpen(false)}
            className="rounded-md p-1.5 text-white/60 transition hover:bg-white/10 hover:text-white md:hidden"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
      ...
      <div className="flex flex-1 flex-col bg-[#f7f5f0]">
        ...
        <button
          ...
          className="flex items-center gap-2 border-b border-black/10 bg-[#0c0d0e] px-4 py-2.5 text-white transition hover:bg-[#1a1b1c] md:hidden"
        >
          <Menu className="h-4 w-4 text-[#e4b653]" />
          <span className="text-[10px] font-semibold uppercase tracking-[0.2em]">Menu</span>
          <span className="ml-auto truncate text-[10px] text-white/50">{activeItem?.label}</span>
        </button>
        {/* Quick-access strip */}
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
          {/* Card header */}
          <div className="flex items-center justify-between gap-2 border-b border-black/10 bg-white px-3 py-2.5 sm:px-4">
            <button onClick={goBack} disabled={!canGoBack}
              className="inline-flex items-center gap-1 rounded-md px-2 py-1.5 text-xs font-medium text-black/50 transition hover:bg-black/5 hover:text-black disabled:cursor-default disabled:opacity-25 sm:px-2.5 sm:gap-1.5">
              <ArrowLeft className="h-3.5 w-3.5" />
              <span className="hidden sm:inline">Back</span>
            </button>
            <p className="flex-1 truncate text-center text-[10px] font-medium uppercase tracking-[0.15em] text-black/40 sm:text-[11px] sm:tracking-[0.25em]">{activeItem?.label}</p>
            <div className="flex items-center gap-2 sm:gap-3">
              <a
                href="https://my-property-intel.base44.app/admin"
                target="_blank"
                rel="noopener noreferrer"
                className="hidden items-center gap-1.5 rounded-md border border-black/15 px-2.5 py-1.5 text-xs font-medium text-black/60 transition hover:bg-black/5 hover:text-black sm:inline-flex"
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            {activeId === "overview" ? (
              <div>
                <div className="p-4 sm:p-6"><WorkflowCreator /></div>
                <AdminOverview />
              </div>
            ) : ActiveComponent ? (
              <ActiveComponent />
