...
const DEFAULT_INPUTS = {
  arv: 250000, as_is_value: 180000, contract_price: 140000, rehab_budget: 30000, rehab_months: 3, holding_months: 5,
  monthly_taxes: 250, monthly_insurance: 120, monthly_utilities: 150, monthly_hoa: 0, commission_rate: 5.5, closing_cost_rate: 3,
  assignment_fee: 13000, seller_mortgage: 100000, seller_liens: 0, hard_money_rate: 8, hard_money_points: 2, down_payment_pct: 10,
  refi_ltv: 75, refi_rate: 7, refi_term: 30, market_rent: 1800, property_tax_pct: 1.1, insurance_annual: 1200,
  management_pct: 8, vacancy_pct: 5, maintenance_pct: 8,
};
  ...
  return (
    <div>
      <Seo
        ...
        jsonLd={{ "@context": "https://schema.org", "@type": "WebApplication", "name": "Real Estate Deal Calculator", "description": "Auto-calculates deal economics with per-person profit breakdown and industry benchmarks.", "url": "https://hiddenpropertyintel.com/deal-calculator", "applicationCategory": "BusinessApplication", "operatingSystem": "Web", "offers": { "@type": "Offer", "price": "0", "priceCurrency": "USD" } }}
      />

      <section className="z-page-hero">
        <p className="z-page-eyebrow"><Calculator size={14} style={{ display: "inline", marginRight: 4 }} /> Deal Calculator</p>
        <h1 className="z-page-h1">Who gets <em>what</em> — and is it fair?</h1>
        <p className="z-page-lead">Auto-calculates every dollar in the deal — seller net proceeds, wholesaler fee, buyer profit, agent commissions, title costs, and transfer taxes — benchmarked against industry standards so everyone walks away fair.</p>
      </section>
      ...
      <section className="z-section-pad" style={{ paddingTop: 8 }}>
        ...
        <div className="z-grid-4" style={{ marginBottom: 24 }}>
          {DEAL_TYPES.map((t) => (
            <button key={t.id} onClick={() => setDealType(t.id)} style={{ border: dealType === t.id ? "2px solid var(--z-blue)" : "1px solid var(--z-border)", background: dealType === t.id ? "#f0f6ff" : "#fff", borderRadius: 12, padding: 18, textAlign: "left", cursor: "pointer", fontFamily: "inherit", transition: ".15s" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <Calculator size={16} style={{ color: dealType === t.id ? "var(--z-blue)" : "var(--z-ink)" }} />
                <b style={{ fontSize: 14, color: "var(--z-ink)" }}>{t.label}</b>
              </div>
              <p style={{ fontSize: 12, color: "var(--z-muted)", margin: "6px 0 0" }}>{t.desc}</p>
            </button>
          ))}
        </div>

        <div style={{ display: "grid", gap: 20, gridTemplateColumns: "1fr 1fr 320px" }} className="z-calc-grid">
...
function calculateDeal(dealType, i) {
  ...
  if (dealType === "wholesale") {
    ...
    metrics = { mao, buyerPrice, buyerProfit, buyerROI, sellerNet, assignmentPctOfSpread: buyerProfit > 0 ? (assignmentFee / buyerProfit) * 100 : 0, dealFairForBuyer: buyerProfit > 0 && buyerROI > 15, dealFairForSeller: sellerNet > 0, assignmentInRange: assignmentFee >= 5000 && assignmentFee <= 20000 };
    ...
    metrics = { mao, totalProjectCost, profit, roi, cashInvested, netSale, dealProfitable: profit > 0, meets70Rule: contractPrice <= mao, roiHealthy: roi > 20 };
    ...
    const mortgagePI = refiAmount * (num(i.refi_rate) / 100 / 12) * 12;
    ...
    metrics = { totalProjectCost, refiAmount, cashOut, cashLeftIn, monthlyCashFlow, cocReturn, annualRent, annualExpenses, mortgagePI, dealProfitable: cashOut > 0, cashFlowPositive: monthlyCashFlow > 0 };
    ...
    people.push({ role: "County / State", name: "Transfer Tax", amount: contractPrice * 0.004, detail: "~0.4%", color: "gray" });
    metrics = { cashInvested, monthlyCashFlow, annualCashFlow, cocReturn, capRate, downPayment, loanAmountHold, annualRent, annualExpenses, cashFlowPositive: monthlyCashFlow > 0, goodCapRate: capRate >= 6 };
  }
